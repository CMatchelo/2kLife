...
function eligible(
  ...
  if (game.status !== "completed" || game.played !== true || !game.stats)
    return false;
  try {
    parseGameDetails({
      status: game.status,
...
export function performanceScore(
  game: Game & { stats: BoxScore; teamScore: number; opponentScore: number },
  prior: (Game & { stats: BoxScore })[],
): number {
  const stats = game.stats;
  const expectation =
    game.starter === true ? 1.1 : game.starter === false ? 0.9 : 1;
  const average = (key: keyof typeof baseline) =>
    prior.length
      ? prior.reduce((sum, g) => sum + g.stats[key], 0) / prior.length
      : baseline[key];
  const relative = (
    key: "points" | "assists" | "rebounds" | "steals",
    floor: number,
  ) => {
  ...
  const parts: [number, number][] = [
    [relative("points", 10), 25],
    [relative("assists", 3), 15],
    [relative("rebounds", 4), 10],
    [relative("steals", 1), 5],
    [
      clamp(
        (average("personalFouls") - stats.personalFouls) /
          Math.max(average("personalFouls"), 2),
        -1,
        1,
      ),
      5,
    ...
    [
      Math.sign(game.teamScore - game.opponentScore) *
        (0.5 +
          0.5 *
            Math.min(Math.abs(game.teamScore - game.opponentScore) / 25, 1)),
      10,
    ],
  ];
  for (const [made, attempted, fallback, weight] of [
    ...
    const target = attempts
      ? prior.reduce((sum, g) => sum + g.stats[made], 0) / attempts
      : fallback;
    // Damp tiny samples so one made shot cannot dominate the evaluation.
    parts.push([
      clamp((stats[made] / stats[attempted] - target) / 0.25, -1, 1) *
        Math.min(stats[attempted] / 5, 1),
      weight,
    ]);
  }
  return clamp(
...
export function recalculateFollowers(
  social: SocialMedia,
  games: Game[],
  savedGameId: string,
): SocialMedia {
  ...
  const tracked = new Set([
    ...(social.trackedGameIds ??
      social.history.flatMap((item) => (item.gameId ? [item.gameId] : []))),
    savedGameId,
  ]);
  const ordered = games
    .filter(eligible)
    .sort((a, b) => a.date.localeCompare(b.date) || a.id.localeCompare(b.id));
  const startingFollowers = clamp(
    Math.round(social.startingFollowers),
    0,
    LIMIT,
  );
  ...
  for (const game of ordered) {
    ...
    const score = performanceScore(
      game,
      ordered.filter((prior) => prior.date < game.date),
    );
    const change = followerChange(followers, score);
    followers += change;
    history.push({
      id:
        social.history.find((item) => item.gameId === game.id)?.id ??
        `followers-${game.id}`,
      gameId: game.id,
      date: game.date,
      change,
      ...
      reason:
        change > 0
          ? "Match performance grew the audience."
          : change < 0
            ? "Poor match performance reduced the audience."
            : "Match performance left the audience unchanged.",
    });
  }
  ...
  return {
    ...
    history: [...history, ...eventHistory].sort(
