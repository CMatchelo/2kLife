...
test("match edits persist, preserve fixtures, validate stats, and replace totals", () => {
  ...
  try {
    const career = store.create({
      requestId: "match-results-test-123456",
      saveName: "Test",
      player: {
        name: "Player",
        position: "PG",
    ...
    assert.throws(() =>
      store.updateGame(career.id, game.id, { ...details, date: "2026-10-20" }),
    );
    assert.throws(() =>
      store.updateGame(career.id, game.id, { ...details, currentPosition: 31 }),
    );
    assert.throws(() =>
      store.updateGame(career.id, game.id, {
        ...details,
        stats: { ...details.stats, points: 5 },
      }),
    );
    assert.equal(store.updateGame("missing", game.id, details), null);
    updated = store.updateGame(career.id, game.id, {
      status: "completed",
      teamScore: 100,
      opponentScore: 90,
      played: false,
