...
async function executable(): Promise<{ file: string; prefix: string[] }> {
  ...
  for (const candidate of ['codex.exe', 'node_modules/@openai/codex/bin/codex.js']) {
...
export function codexProvider(run: Run = runCodex): Provider {
  return {
    ...
    async sponsorApproach(context) {
      ...
      try {
        const schemaFile = join(directory, 'schema.json'); const outputFile = join(directory, 'result.json');
        await writeFile(schemaFile, JSON.stringify(sponsorApproachSchema), { mode: 0o600 });
        await run(['exec', '--ignore-user-config', '--ephemeral', '--skip-git-repo-check', '--sandbox', 'read-only', '--json', '--output-schema', schemaFile, '--output-last-message', outputFile, sponsorApproachPrompt(context)], directory);
        return JSON.parse(await readFile(outputFile, 'utf8'));
      } finally { await rm(directory, { recursive: true, force: true }); }
    },
    async interview(context) {
      ...
      try {
        const schemaFile = join(directory, 'schema.json'); const outputFile = join(directory, 'result.json');
        await writeFile(schemaFile, JSON.stringify(interviewSchema), { mode: 0o600 });
        await run(['exec', '--ignore-user-config', '--ephemeral', '--skip-git-repo-check', '--sandbox', 'read-only', '--json', '--output-schema', schemaFile, '--output-last-message', outputFile, interviewPrompt(context)], directory);
        return JSON.parse(await readFile(outputFile, 'utf8'));
      } finally { await rm(directory, { recursive: true, force: true }); }
    },
    async extract(images, context) {
      ...
      try {
        const args = ['exec', '--ignore-user-config', '--ephemeral', '--skip-git-repo-check', '--sandbox', 'read-only', '--json'];
        for (const [index, image] of images.entries()) {
          const file = join(directory, `image-${index + 1}.${image.mediaType.split('/')[1]}`);
        ...
        args.push('--output-schema', schemaFile, '--output-last-message', outputFile, extractionPrompt(context));
    ...
    async check() {
      await run(['--version']);
      try { await run(['login', 'status']); }
      catch (error) {
        if (/not logged in|not signed in/i.test(String((error as { message?: string }).message))) return { configured: false, message: 'Not signed in. Run codex login in your terminal, then check again.' };
        throw error;
      }
      return { configured: true, message: 'Codex has saved authentication. Test to verify access. Subscription sign-in uses Codex allowance; API-key sign-in uses API billing.' };
    },
    async test() {
      const directory = await mkdtemp(join(tmpdir(), '2klife-test-'));
      try {
        const output = await run(['exec', '--ignore-user-config', '--ephemeral', '--skip-git-repo-check', '--sandbox', 'read-only', '--json', TEST_PROMPT], directory);
        ...
        if (!events.some(event => event.type === 'turn.completed') || !events.some(event => event.type === 'item.completed' && event.item?.type === 'agent_message' && event.item?.text?.trim())) throw new Error('Incomplete response');
      } finally { await rm(directory, { recursive: true, force: true }); }
    },
  };
}
